...
class PopulationsStandardCard extends StatelessWidget {
  const PopulationsStandardCard({
    Key? key,
    required this.ref,
    required this.data,
  }) : super(key: key);
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [FemaleCountsColumnWidget(data: data)],
                      ),
                    ),
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceAround,
                        children: [MaleCountColumnWidget(data: data)],
                      ),
                    )
                  ],
                )),
...
class FemaleCountsColumnWidget extends StatelessWidget {
  const FemaleCountsColumnWidget({
    Key? key,
    required this.data,
  }) : super(key: key);
  ...
  String getFemaleCountOneAverage() {
  ...
  String getFemaleCountTwoAverage() {
    List<int> observations = [];
    if (data.InspectionChildren != null) {
      if (data.InspectionChildren!.isNotEmpty) {
        for (var inspection in data.InspectionChildren!) {
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Row(children: [
            const Text('One: ', textAlign: TextAlign.center),
            Text(getFemaleCountOneAverage())
          ]),
        ),
        Padding(
          padding: const EdgeInsets.all(2.0),
          child: Row(children: [
            const Text('Two: ', textAlign: TextAlign.center),
            Text(getFemaleCountTwoAverage())
          ]),
        ),
      ],
    );
...
class MaleCountColumnWidget extends StatelessWidget {
  const MaleCountColumnWidget({
    Key? key,
    required this.data,
  }) : super(key: key);
  ...
  String getMaleCountAverage() {
    List<int> observations = [];
    if (data.InspectionChildren != null) {
      if (data.InspectionChildren!.isNotEmpty) {
        for (var inspection in data.InspectionChildren!) {
          if (inspection.populationsStandardSeedCornFormObject?.maleCount !=
              null) {
            observations.add(
                inspection.populationsStandardSeedCornFormObject!.maleCount);
          }
        }
      }
    }
